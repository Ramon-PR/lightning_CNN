...
class ZeaDataset_C1(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.target_transform:
            ...
            temp  = torch.cat((image, im_mask_col),dim=-2)
            temp = self.target_transform(temp)
            
            # Recuperate the transformed target and mask
            target, im_mask_col = torch.split(temp, H, dim=-2)
            image = target
        ...
        return image, target, im_mask_col


class ZeaDataset_C3(data.Dataset):
